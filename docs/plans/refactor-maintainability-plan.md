...
- [x] 批次 A：抽离 `gpu/uniforms.ts` 与 `interaction/hitTest.ts`。
- [x] 批次 B：渲染层拆为 `pipelines.ts`、`offscreenTargets.ts`、`renderPasses.ts`。
- [x] 批次 C：入口拆为 `app/bootstrap.ts` 与 `app/runtime.ts`。
