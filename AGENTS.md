...
- 提交信息强制使用 Conventional Commits 1.0.0：`<type>[optional scope]: <description>`。
- `type` 建议使用：`feat`、`fix`、`docs`、`style`、`refactor`、`test`、`build`、`ci`、`chore`、`perf`。
- `scope` 使用英文 kebab-case（如 `gpu-renderer`、`interaction`）；`description` 使用简体中文，简短明确。
- 破坏性变更使用 `!` 或在正文写 `BREAKING CHANGE:`（例如 `feat(state)!: 调整玻璃状态结构`）。
- 示例：`fix(hit-test): 修复边缘命中判定偏移`、`refactor(renderer): 拆分渲染通道编码逻辑`。
