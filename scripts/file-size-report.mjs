 ...
 */
async function getFileLineStat(fullPath) {
  const text = await fs.readFile(fullPath, "utf8");
  const lineCount = text.length === 0 ? 0 : text.split(/\r?\n/).length;
  return { fullPath, lineCount };
}

/**
 * 生成排行榜。
 * @param {{fullPath:string,lineCount:number}[]} allFiles
 * @param {".ts"|".wgsl"} ext
 * @param {number} limit
 */
 ...
 * 打印排行榜。
 * @param {string} title
 * @param {{rank:number,relativePath:string,lineCount:number}[]} rows
 */
function printTable(title, rows) {
  ...
  for (const row of rows) {
    ...
    console.log(`${rank}. ${lines} 行 | ${row.relativePath}`);
  }
}
...
async function main() {
  try {
    ...
    const allFiles = await Promise.all(
      filePaths.map((filePath) => getFileLineStat(filePath)),
    );
    ...
    printTable("行数最多的 10 个 .ts 文件（降序）", topTs);
    printTable("行数最多的 5 个 .wgsl 文件（降序）", topWgsl);
  } catch (error) {
